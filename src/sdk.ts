...
const contractAddressDOM = <HTMLInputElement>(document.getElementById('contractAddress'));
const tokenIdDOM = <HTMLInputElement>(document.getElementById('tokenId'));
...
const checkLicense = async () => {
  ...
  const contract = new Contract(License.abi, contractAddress);
  contract.setProvider(web3.currentProvider);

  let owner;
  await contract.methods.ownerOf(tokenId).call().then((result) => {
    updateDOM(responseDOM, result as any as string);
    owner = result;
  });

  if (owner === web3.utils.toChecksumAddress(account)) {
    let metadata;
    await contract.methods.getMetadata().call().then((result) => {
      metadata = JSON.parse(result as any as string);
    });
